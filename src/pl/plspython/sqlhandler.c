...
static plspython_handler handlertab = NULL;
...
static plspython_result create_sql_result(void);
...
static plspython_result
create_sql_result() {
    plspython_result result;
    int              i, j;
    ...
    for (j = 0; j < result->cols; j++) {
        ...
        result->types[j] = pstrdup(NameStr(typstr->typname));
