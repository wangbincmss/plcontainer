...
static void send_result(plspython_result res);
...
void
plspython_channel_initialize(PGconn_min *conn) {
    min_conn = conn;
}

bool
plspython_channel_initialized() {
    return min_conn != NULL;
}

void
plspython_channel_send(message msg) {
    switch (msg->msgtype) {
    case MT_CALLREQ:
        send_call((callreq)msg);
        break;
    case MT_RESULT:
        send_result((plspython_result)msg);
        break;
    case MT_EXCEPTION:
...
message
plspython_channel_receive() {
    message msgret = NULL;
    char    type   = receive_char();
    //elog(DEBUG1, "plspython_channel_receive type = %c", type);
...
static void
send_result(plspython_result res) {
    int i, j;
    message_start();
    send_char(MT_RESULT);
